...
menu_items = [
    ...
    "Hasil Cluster",
    "Proyeksi Pensiun & Rekomendasi Pegawai",
...
# ------------------ PROYEKSI PENSIUN ------------------
    ...
    total_pensiun = len(df_pensiun)
    st.markdown(f"📋 **Total Pegawai yang Akan Pensiun ≤ {batas_pensiun} Tahun: {total_pensiun} orang**")
...
# ------------------  ------------------
    ...
    with st.expander("📊 Heatmap Rata-rata Usia per Pendidikan Akhir dan Level Jabatan"):
        ...
        ax = sns.heatmap(heatmap_data, annot=True, fmt=".1f", cmap="YlGnBu", linewidths=.5, cbar_kws={"label": "Rata-rata Usia"})

    # ➕ Tambahkan keterangan Level Jabatan di atas heatmap
        plt.text(-1.6, -1.8, 
             "Level Jabatan:\n0.3 = Pemula, 0.5 = Terampil, 0.7 = Mahir, 0.9 = Penyelia,\n1.0 = Ahli Pertama, 2.0 = Ahli Muda, 3.0 = Ahli Madya, 4.0 = Ahli Utama",
             fontsize=9, 
    ...
    with st.expander("📊 Heatmap Rata-rata Usia berdasarkan Rentang Usia dan OPD"):
        ...
        plt.xlabel('Rentang Usia', fontsize=13)
        plt.ylabel('OPD', fontsize=13)

        # Perbesar dan rapikan label Rentang Usia
